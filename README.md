
# 🎮 Traducciones El Cuartel Blasters

Únete a nuestro Discord:  https://discord.gg/fyqF6dhfUQ

## Traducción al Español — Yo-kai Watch Blasters 2

Bienvenidos al proyecto de traducción al español de **Yo-kai Watch Blasters 2**.

Nuestro equipo ha trabajado durante meses para alcanzar entre un **70% y 80% de la traducción total**, basándonos en la versión inglesa creada por **Light y Sofuto**.

El proyecto comenzó de forma individual y, con el tiempo, el equipo creció.
A pesar de distintos retrasos y del tiempo limitado, todos cumplieron con la entrega y edición de archivos (traducción, ajustes y mejoras).

⚠️ **Esto no es el producto final.**
Se ha pulido bastante para ofrecer una experiencia completa, pero aún faltan elementos por traducir y ajustar.

📅 **El contenido restante se seguirá publicando semanalmente.**

---

# 📊 Progreso de Traducción

## 📝 Textos

* **Menús** → 100%
* **Texturas de menús** → 99%
* **Historia** → 90% *(faltan 2 capítulos)*
* **Diálogos de personajes** → 90%
* **Textos post-jefe** → 90%

---

## 🔊 Audio

* **Voces de Yo-kai** → 80% *(faltan doblajes)*
* **Voces de Ataque** → 80% *(faltan doblajes)*

---

## 🖥️ Interfaz / Sistema

* Video de presentación → Traducido
* Banner giratorio 3DS → Traducido
* Íconos del menú 3DS → Localizados
* Título del juego → Localizado
* Imágenes de ayuda → Traducidas

---

## 🌍 Localización General

* Íconos, objetos, ropa y personajes → Localizados
* Texturas varias *(puertas, logos, etc.)* → Editadas
* Presentación de jefes → Traducida
* Presentación de mazmorras → Traducida

---

## 🎯 Modos / Extras

* **Arena del viajero** → 90%

---

# 🎬 Demostración

📺 Comparación Inglés vs Español:

[https://youtu.be/aeW33WVZFjA](https://youtu.be/aeW33WVZFjA)

---

# 📦 Contenido Faltante

El contenido faltante se publicará:

📅 **Cada lunes**

<img width="2400" height="1348" alt="BlastersTVisualArtwork" src="https://github.com/user-attachments/assets/a7253a72-1aa6-4bef-adca-45a30bbf621f" />

Además:

✨ Se añadirá contenido adicional semanal
📍 Se seguirá una hoja de ruta progresiva
⚠️ Las fechas específicas pueden variar

---

# 🎙️ Próximamente

📢 El **20 de marzo** iniciamos:

➡️ **Doblaje en Español**

Cada semana se añadirá:

* Nuevo capítulo de historia
* Más contenido jugable
* Más doblajes
* Mejoras adicionales

---

# ⚙️ Mejoras y Cambios Implementados

* Se añadió el **mod de reequilibrio de amuletos** creado por Light
  [https://gamebanana.com/mods/510623](https://gamebanana.com/mods/510623)

* Se añadió el **mod de tienda infinita mejorada** creado por Light
  [https://gamebanana.com/mods/619683](https://gamebanana.com/mods/619683)

* Ajuste de nombres de 2 auras

* Reequilibrio menor en objetos

* Mejoras de calidad de vida

* Optimización general del juego

📢 **Más contenido será anunciado próximamente.**

---

# 🧪 Estado del Parche

Podrán disfrutar de una experiencia:

✅ **Visualmente completa**
✅ **Jugable desde inicio hasta capítulo 6**
⚠️ **Faltan 4 capítulos por traducir**

### 📖 Progreso de Historia

* Capítulos traducidos → **9 / 11**

📌 Si llegas al capítulo 9:

➡️ **DETENTE y espera al siguiente lunes**
➡️ Cada semana se añadirá un nuevo capítulo

---

# 🐞 Reporte de Errores

Si encuentras:

* Errores de traducción
* Texturas mal colocadas
* Texto mal alineado
* Problemas visuales
* Frases que puedan mejorarse

📢 **Repórtalo en la sección correspondiente del repositorio o en nuestro Discord.**

Esto nos permitirá:

✔️ Mejorar la calidad
✔️ Corregir errores
✔️ Pulir la experiencia

---

# 🙏 Agradecimientos y Créditos

## 🎮 Creadores Base

**Light y Sofuto**
Creadores del mod original.

Sin su trabajo, este proyecto no habría sido posible.

---

## 🧰 Soporte Técnico

**YO NEW 3DS**

**DARKVERD**

**NONO**

*(Equipo B2 Francés)*

Apoyo constante en herramientas, archivos y asistencia técnica.

---

## 👨‍💻 Equipo Principal

**@VictorFco**
Creación del servidor y gran apoyo en:

* Edición de textos
* Contenido
* Texturas
* Archivos de guardado

---

**@BryanAG**

* Edición y creación de videos
* Tráiler de revelación
* Traducción
* Edición de archivos

---

**@mayo**

* Herramientas
* Edición de audios
* Modificación de archivos `.bin`
* Traducción

---

**@🧪DIVINIUM115🧪**

* Diseño
* Traducción
* Texturas
* Audios

---

# 🎙️ Actores de Voz

El casting continúa abierto.

Quienes ya han participado:

* **@Pokemaster Latino**
  Rogencio — “¡Eso! ¡Eso!”

* **@Metalzero**
  Dandoriel
  Ninja Maizo
  Narrador de medallas
  Voz del tráiler

* **@BryanAG**
  Rúedon
  Segárbol
  Narciso II

* **@máximo123g**
  Wibblejiro

* **@valto_m**
  Retanquetanke

* **@🧪DIVINIUM115🧪**
  Depreyama

---

# 📌 Conclusión

Hicimos todo lo posible para que:

🎮 La experiencia sea funcional
🧭 La navegación sea clara
🧩 El juego sea disfrutable

Aunque:

♾️ **No todo es perfecto todavía**

Se publicarán próximamente los **3 defectos conocidos del parche**, los cuales:

✔️ No afectan la jugabilidad
✔️ Son detalles menores

---

# 💬 Feedback

Tu opinión es importante.

Si deseas:

* Mejorar frases
* Corregir errores
* Ajustar textos
* Reportar problemas

📢 **Avísanos en la sección de errores del repositorio o en nuestro Discord.**

---

# 🚀 Esto apenas empieza

Gracias por esperar 🫂

Esperamos que disfruten esta traducción.
Y recuerda:

👉 **No será la última.**


